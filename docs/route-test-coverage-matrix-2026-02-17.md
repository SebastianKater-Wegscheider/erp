...
| sourcing | scrape/analyze/convert/discard, scheduler agents | Backend | 14 (`test_sourcing_flows.py`, `test_sourcing_scheduler_agents.py`) | 0 | 0 (automated), manual Playwright audit done | 16 | 1 |
...
## Luecken (priorisiert)
1. Sourcing hat weiterhin keine automatisierte E2E-Abdeckung fuer run/convert/discard (nur manuell verifiziert).
2. Sales-E2E ist aktuell instabil (Timeout in `sales.spec.ts` beim Auftrag-Dialog).
3. Marketplace-E2E ist aktuell fragil wegen ambigem READY-Selector (`marketplace.spec.ts` strict-mode violation).
4. Inventory hat noch keinen dedizierten E2E-Smoke fuer Status-/Pricing-UI.
...
## Live-Run Status (2026-02-17)
- Automated E2E with local compose credentials: 4/6 gruen, 2/6 rot (`marketplace.spec.ts`, `sales.spec.ts`).
- Manual Playwright sourcing run: scrape -> convert -> discard technisch funktional; UX/operability findings in tech-review dokumentiert.
